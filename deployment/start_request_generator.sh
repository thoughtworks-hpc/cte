...
while true; do
  ...
  # 判断上一轮是否有残留
  cte_trades_count=$(curl -GET 'http://172.30.28.30:8086/query?pretty=true' -s --data-urlencode "db=trade_manager" --data-urlencode "q=SELECT count(*) FROM cte_trades" | python -c 'import json,sys;obj=json.load(sys.stdin); print(obj["results"][0]["series"][0]["values"][0][1])')
  akka_te_trades_count=$(curl -GET 'http://172.30.28.30:8086/query?pretty=true' -s --data-urlencode "db=trade_manager" --data-urlencode "q=SELECT count(*) FROM akka_te_trades" | python -c 'import json,sys;obj=json.load(sys.stdin); print(obj["results"][0]["series"][0]["values"][0][1])')
  ...
  while true; do
    ...
    if [ $count1 == $count2 ]; then
      ...
      echo "[IMPORTANT] When cte db available, cte_trades_count: ${cte_trades_count}"
      echo "[IMPORTANT] When cte db available, cte_trades_count: ${cte_trades_count}" >>/tmp/log/long_run_status.log
      echo "[IMPORTANT] When cte db available, akka_te_trades_count: ${akka_te_trades_count}"
      echo "[IMPORTANT] When cte db available, akka_te_trades_count: ${akka_te_trades_count}" >>/tmp/log/long_run_status.log
      break
    fi
  ...
  while true; do
    ...
    if [ $count1 == $count2 ]; then
      ...
      echo "[IMPORTANT] When akka db available, akka_te_trades_count: ${akka_te_trades_count}"
      echo "[IMPORTANT] When akka db available, akka_te_trades_count: ${akka_te_trades_count}" >>/tmp/log/long_run_status.log
      break
    fi
